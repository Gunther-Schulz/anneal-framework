...
considered and rejected: the impl-phase integrity check
(`core.md` §4.2) already refuses-and-surfaces on a non-clean
container before dispatching an in-place unit, so an
`on-instance-start` gate would duplicate an existing framework
check — the Additive-reflex anti-pattern. The slot stays available
via the enable mechanism above.)
