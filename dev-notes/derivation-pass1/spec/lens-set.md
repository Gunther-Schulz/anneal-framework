...
- **Lossy rendering** and **over-claimed verification** are watched
...
Missed-dependents, Unenforced-rules, Undefined-shorthand) are
...
- **Scope:** any rule-text the cycle adds or amends — a new clause,
  an expanded section, a new example. Fires on the diff's added
...
- **Question:** is the principle this cycle states or amends already
  ...
  two copies that can drift apart? The basis is a wrap-tolerant grep
  for the principle's distinctive tokens across the whole corpus
  (`bindings.md` exhaustive-search binding). If a copy exists, the
  cycle resolves to one canonical statement + cross-references, not a
  second copy.
...
- **Question:** for a rule the cycle changes, removes, or replaces,
  ...
  the changed contract? Dependents hide behind: **cross-references**
  (a `§N`/file-name pointer to the changed rule), **paraphrased
  restatements** (the same rule worded differently elsewhere —
  invisible to a verbatim grep), **rendered clauses** (the plugin
  paraphrase of the rule), **closed-set members** (the rule names a
  member enumerated in more than one file), and **glossary/defined-
  term uses** (the rule uses a term whose definition the change
  affects). A grep on one shape only, declared as the dependent set,
  is missing its true-unit basis — the silent-substitution shape
  (`core.md` §3.2).
- **Scope:** any change, removal, or replacement of an existing rule
  ...
  paraphrased restatement or a rendered clause that a verbatim grep
  cannot exhibit is a dependent that resolves through paraphrase —
  recorded [CONDITIONAL] with its basis the dependent exercised by
  the render-fidelity / coherence checks (`core.md` §3.2.2,
  `bindings.md` Verification battery), not a verbatim grep.
...
- **Question:** does the cycle introduce or rely on a shorthand term
  ...
  wrap-tolerant grep establishing whether the term is defined
