...
### ▶ Counter ground truth (groomed 2026-07-16 — operator-called): "N open" narrative counters retired
The running "N open" tallies woven through the blocks below had **drifted against the on-disk
inventory** (flagged by the 2026-07-16 booking review, drift ≥2026-07-10: two divergent counter
chains — the READ-FIRST chain ended at "46 open", the session-15 chain at ~61 — both from-memory
increments, never recounted). Ground truth at this grooming: **64 open items** on disk
(19 READY · 22 DESIGN · 10 PARKED · 11 PARTIAL · 2 GATED, per the Convention's self-audit grep).
**Per-block "N open" counters are hereby historical and no longer maintained.** The convention
already says it — the folder IS the index: count with `ls *.md | grep -v README | wc -l`, tally
states with the self-audit grep. Do not append running counters to future blocks.

### ▶ Model-axis datapoint (2026-07-16, later — READ FIRST): first false-accept evidence for same-tier verify; DISPOSITIONED same day — fork (a) narrow
The pbs instance delivered the reopen-trigger `verify-model-diversity.md` has been holding the
option open for: a build day ran opus-4.8 (operator believed fable-5; caught via the per-commit
`Co-Authored-By` model trailer), and three fresh-context **cross-tier** reviews over that day's
work split cleanly — same-tier anneal passes had caught the attention class, but **four
judgment-class defects passed same-tier verify clean and fell only cross-tier** (legal fencepost,
overclaiming monitor message, Ermessen-vs-automatism in shipped text, stale-comment-over-data
grounding). First false-accept-side measurement (prior evidence was true-positive-only). Detail:
`verify-model-diversity.md` §2026-07-16. **Operator disposition (2026-07-16, in-chat, evening
session): fork (a) NARROW** — verify carve-out from the model-tier floor, scoped to the
judgment-class leg (intent-falsification; checker tier ≥ actor tier / cross-model where
available); mechanical/lens legs stay in the floor. Item → [DESIGN]; **runs as ONE gated-kernel
anneal-dev campaign with the sibling** (shared datapoint + verify-machinery locus). Sibling filed:
`judgment-to-mechanical-lens-candidates.md` [DESIGN] — three lens/discipline candidates
(domain-claim re-derivation incl. data-beats-commentary, worked-boundary-example obligation,
closed-set sweep) that convert judgment-class into attention-class → ~~**46 open.**~~ *(stale
chain counter — see ▶ Counter ground truth above; disk said 64.)*

### ▶ Cross-instance datapoint (2026-07-16 — READ FIRST): a second deployed instance confirms the design-time-depth-gap thesis
A corpus-evolution run (`reviewer-regel-kanal`) in a **separate deployed anneal-dev instance** (the
Planungsbüro-Schulz pbs rule-corpus, bound in-context — not clippy) independently reproduced the
`design-decision-implication-depth-gaps` shape (broken-basis-not-propagated: a 3→4-way classify change
not traced to a dependent decision's enumeration; caught by the next fresh-context convergence pass) and
its **convergence-cost corollary**, operator-diagnosed live: iterated convergence cost is the visible
*symptom* of design-time depth gaps, **not** a case for capping iterations — a **counter-datapoint to
`clippy-category-b-recalibration.md` #3** ("cheapen convergence to one cycle, not iterated"). Detail +
the cycle-chain audit: `design-decision-implication-depth-gaps.md` §Second-run confirmation; reinforces
